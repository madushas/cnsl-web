...
export default function AdminEventsPage() {
  ...
  useEffect(() => {
    ...
    setStatus(
      st === "published" || st === "draft" ? (st as "published" | "draft") : "",
    );
    const tf = sp.get("timeframe") || "";
    setTimeframe(
      tf === "upcoming" || tf === "past" ? (tf as "upcoming" | "past") : "",
    );
  ...
  async function togglePublished(row: EventRow) {
    setRows((rs) =>
      rs.map((r) =>
        r.slug === row.slug ? { ...r, published: !row.published } : r,
      ),
    );
    const res = await fetch(`/api/events/${row.slug}`, {
      method: "PATCH",
    ...
    <div className="px-4 lg:px-6 space-y-6">
        ...
        {rows.map((ev) => (
          ...
          >
            ...
            <div className="text-xs text-muted-foreground mt-0.5">
              ...
              {ev.city ? `· ${ev.city}` : ""} {ev.venue ? `· ${ev.venue}` : ""}
            </div>
            <div className="flex items-center justify-between mt-2">
              <div className="flex items-center gap-2">
                <Switch
                  checked={!!ev.published}
                  onCheckedChange={() => togglePublished(ev)}
                />
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        ...
                        delete the event &quot;{ev.title}&quot; and remove its data.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                      <AlertDialogCancel>Cancel</AlertDialogCancel>
                      <AlertDialogAction onClick={() => del(ev.slug)}>
                        Delete
                      </AlertDialogAction>
                    </AlertDialogFooter>
                  </AlertDialogContent>
      ...
      <div className="flex flex-wrap items-center gap-4 bg-muted/30 p-4 rounded-lg border border-border">
        <div className="flex items-center gap-2">
          ...
          <div className="relative">
            ...
            <Input
              id="search"
              placeholder="Search title, slug, city, venue"
              className="pl-8 w-72"
              value={q}
        ...
        <div className="flex items-center gap-2">
          ...
          <Select
            ...
            onValueChange={(v) => {
              setStatus(
                v === "published" ? "published" : v === "draft" ? "draft" : "",
              );
              setPage(1);
            }}
          >
        ...
        <div className="flex items-center gap-2">
          ...
          <Select
            ...
            onValueChange={(v) => {
              setTimeframe(
                v === "upcoming" ? "upcoming" : v === "past" ? "past" : "",
              );
              setPage(1);
            }}
          >
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              ...
                              permanently delete the event &quot;{ev.title}&quot; and
                              remove its data.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
      ...
      <div className="flex items-center justify-between text-sm bg-muted/30 p-4 rounded-lg border border-border">
        <div className="text-muted-foreground font-medium">
          Page {page} of {pageCount} · {total} total events
        </div>
        <div className="flex items-center gap-2">
          <Select
