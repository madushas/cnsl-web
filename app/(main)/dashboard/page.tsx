...
async function getMyRsvps(userId?: string, email?: string | null): Promise<RsvpRow[]> {
  ...
  if (!user) {
    return (
      <div className="container mx-auto px-4 py-12">
        ...
        <Link href="/signin" className="inline-flex items-center rounded-md bg-primary px-4 py-2 text-primary-foreground text-sm">Sign in</Link>
      </div>
    )
  }
      ...
      <header>
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
          <div>
            ...
            <p className="text-muted-foreground">Here are your upcoming and past RSVPs.</p>
          </div>
          <Link href="/account" className="inline-flex items-center rounded-md bg-primary px-3 py-2 text-primary-foreground text-sm">Manage profile</Link>
        </div>
      </header>
              ...
              <li key={r.id} className="rounded-md border p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">{new Date(r.date).toLocaleString()} — {r.venue} · {r.city}</div>
                    <div className="mt-1 text-xs">Status: <span className="font-medium">{r.status}</span></div>
                    {r.ticket_number ? (
                      <div className="mt-2 text-xs">
                        <div className="font-medium">Ticket</div>
                        <div className="text-muted-foreground">#{r.ticket_number}</div>
                        {r.qr_code && (
                          <div className="mt-2">
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img src={r.qr_code} alt={`QR for ${r.title}`} className="h-32 w-32 rounded border" />
                          </div>
                        )}
                      </div>
                    ) : (
                      <div className="mt-2 text-xs text-muted-foreground">Ticket will appear here when available.</div>
                    )}
                  </div>
                  <div className="flex gap-2 sm:self-start">
                    <Link href={`/events/${r.slug}`} className="text-sm text-primary underline">View event</Link>
                  </div>
                </div>
              </li>
            ))}
            ...
            {past.map(r => (
              <li key={r.id} className="rounded-md border p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex gap-2 sm:self-start">
                    <Link href={`/events/${r.slug}`} className="text-sm text-primary underline">View details</Link>
                  </div>
                </div>
              </li>
            ))}
