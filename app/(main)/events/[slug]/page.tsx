...
async function getEvent(slug: string): Promise<EventDetail | null> {
  ...
  const topics = await db.select().from(schema.eventTopics).where(eq(schema.eventTopics.eventId, event.id))
  const speakers = await db.select().from(schema.eventSpeakers).where(eq(schema.eventSpeakers.eventId, event.id))
  ...
  return {
    ...
    speakers: speakers.map(s => ({ name: s.name, title: s.title, topic: s.topic })),
    registered: Number(countRow?.c || 0),
  }
}

export async function generateMetadata(props: { params: Promise<{ slug: string }> }, _parent?: any): Promise<Metadata> {
  ...
  return {
    ...
    description: event.description || `Join us for ${event.title} at ${event.venue}, ${event.city}`,
...
export default async function EventDetailPage(props: { params: Promise<{ slug: string }> }) {
  ...
  if (user?.id || user?.email) {
    const existingRsvp = await db.select({ id: schema.rsvps.id })
      .from(schema.rsvps)
      .where(
        and(
          eq(schema.rsvps.eventId, event.id),
  ...
  const jsonLd = {
    ...
    image: event.image ? [ (event.image.startsWith('http://') || event.image.startsWith('https://')) ? event.image : `${siteUrl}${event.image}` ] : [],
    organizer: {
      '@type': 'Organization',
