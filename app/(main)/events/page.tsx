...
export const metadata: Metadata = {
  title: "Events | CNSL",
  description:
    "Discover upcoming community meetups and special events by Cloud Native Sri Lanka.",
...
async function getEvents(): Promise<EventItem[]> {
  const rows = await db
    ...
    .where(
      and(
        ...
        sql`${schema.events.deletedAt} IS NULL`,
      ),
    )
  ...
  return rows.map((e) => ({
    ...
    speakers: speakers
      ...
      .map((s) => ({ name: s.name, title: s.title, topic: s.topic })),
      ...
      >
        <div className="mb-12">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl">
            Join us for community meetups, workshops, and tech talks. Connect
            with fellow developers and learn from industry experts.
          </p>
        </div>
