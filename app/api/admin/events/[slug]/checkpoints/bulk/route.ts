...
export async function POST(
  ...
  try {
    ...
    if (validRsvpIds.length !== rsvpIds.length) {
      return apiError(
        "VALIDATION_ERROR",
        "Some RSVPs do not belong to this event",
        400,
      );
    }
    ...
    for (let i = 0; i < rsvpIds.length; i++) {
      ...
      try {
        if (action === "mark") {
          ...
          // Undo checkpoint
          const ok = await deleteCheckpointScan(
            rsvpId,
            eventId,
            checkpointType,
          );
        ...
        errorCount++;
        console.error(
          `[Bulk Checkpoint] Error processing RSVP ${rsvpId}:`,
          error,
        );
      }
