...
export async function POST(
  ...
  const endpoint = (() => {
    try {
      ...
      return "/api/admin/events/[slug]/notify";
    }
  })();
  ...
  try {
    ...
    if (!rateCheck.allowed) {
      ...
      return NextResponse.json(
        {
          error:
            "Rate limit exceeded. Please wait before sending another bulk email.",
          resetAt: rateCheck.resetAt.toISOString(),
        },
        { status: 429 },
      );
    ...
    logger.info("admin.email.notify.start", {
      slug,
      endpoint,
    ...
    if (!targets.length)
      return NextResponse.json(
        { error: "No targets found for filters" },
        { status: 400 },
      );
    ...
    logger.info("admin.email.notify.ok", {
      slug,
      jobId,
    ...
    const duration = Date.now() - t0;
    logger.error("admin.email.notify.error", {
      endpoint,
      slug: slugForLog,
      error: e?.message,
    });
