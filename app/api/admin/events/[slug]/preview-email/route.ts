...
export async function POST(
  ...
  const endpoint = (() => {
    try {
      ...
      return "/api/admin/events/[slug]/preview-email";
    }
  })();
  ...
  try {
    ...
    logger.info("admin.email.preview.start", {
      slug,
      endpoint,
      filters: { q, status, ids: ids.length },
    });
    ...
    const rows = await db
      ...
      .leftJoin(
        schema.users,
        eq(schema.users.authUserId, schema.rsvps.accountId),
      )
      .where(whereExpr)
    ...
    await logAudit({
      ...
      newValues: {
        subject,
        hasHtml: !!html,
        filters: { status, q, ids: ids.length },
      },
      ipAddress: req.headers.get("x-real-ip"),
    });

    const duration = Date.now() - t0;
    logger.info("admin.email.preview.ok", {
      slug,
      total: Number(total) || 0,
      duration,
    });
    logger.api("POST", endpoint, 200, duration, {
      userId: admin?.id ? String(admin.id) : undefined,
      slug,
      total: Number(total) || 0,
    });
    return NextResponse.json({
    ...
    const duration = Date.now() - t0;
    logger.error("admin.email.preview.error", {
      endpoint,
      slug: slugForLog,
      error: e?.message,
    });
