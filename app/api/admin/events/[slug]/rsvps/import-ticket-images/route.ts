...
export async function POST(
  ...
  const endpoint = (() => {
    try {
      ...
      return "/api/admin/events/[slug]/rsvps/import-ticket-images";
    }
  })();
  ...
  try {
    ...
    if (ct.includes("text/csv")) {
      ...
      if (!headers.length)
        return apiError("BAD_REQUEST", "CSV has no header", 400);
      ...
      records = rows.map((r) =>
        Object.fromEntries(
          r.map((v, idx) => [normalized[idx] || `col_${idx}`, v]),
        ),
      );
    } else {
      ...
      if (!Array.isArray(body))
        return apiError(
          "BAD_REQUEST",
          "Expected CSV or JSON array of mappings",
          400,
        );
      records = body;
    }

    logger.info("admin.rsvps.importTickets.start", {
      slug,
      endpoint,
    ...
    if (records.length > 1000)
      return apiError("BAD_REQUEST", "Too many rows (max 1000)", 413);
    ...
    for (const rec of records) {
      ...
      const ticketIn = sanitizeTicketNumber(
        rec.ticketnumber ?? rec.ticket_number ?? rec.ticket,
      );
        ...
        const row = await db
          ...
          .where(
            and(
              ...
              eq(schema.rsvps.ticketNumber, ticketIn),
            ),
          )
          .limit(1);
        match = row[0] || null;
      ...
      const finalTicket =
        ticketIn || match.ticketNumber || generateTicketNumber(event.id);
      const finalQr =
        qrIn || match.qrCode || `${event.id}|${finalTicket}|${match.id}`;
      ...
      if (!Object.keys(set).length) {
        results.push({ key, updated: false, reason: "nothing_to_update" });
        continue;
      }
      ...
      try {
        await db
          .update(schema.rsvps)
          .set(set)
          .where(eq(schema.rsvps.id, match.id));
        updated++;
        results.push({ key, updated: true });
      } catch (err: any) {
        const msg = (err?.message || "").toLowerCase();
        const reason =
          msg.includes("unique") && msg.includes("ticket")
            ? "ticket_conflict"
            : "db_error";
    ...
    logger.info("admin.rsvps.importTickets.ok", {
      slug,
      updated,
      total: records.length,
      duration,
    });
    logger.api("POST", endpoint, 200, duration, {
      slug,
      updated,
      total: records.length,
    });
    ...
    const duration = Date.now() - t0;
    logger.error("admin.rsvps.importTickets.error", {
      endpoint,
      slug: slugForLog,
      error: e?.message,
    });
    logger.api("POST", endpoint, 400, duration, {
      slug: slugForLog,
      error: e?.message,
    });
