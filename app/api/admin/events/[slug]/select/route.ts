...
export async function POST(req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const [event] = await db.select({ id: schema.events.id, capacity: schema.events.capacity }).from(schema.events).where(eq(schema.events.slug, slug)).limit(1)
    if (!event) return apiError('NOT_FOUND', 'Event not found', 404)

    // Ensure all RSVPs belong to event
    const rows = await db.select({ id: schema.rsvps.id }).from(schema.rsvps).where(and(eq(schema.rsvps.eventId, event.id), inArray(schema.rsvps.id, rsvpIds)))
    const validIds = rows.map(r => r.id)
    if (!validIds.length) return apiError('BAD_REQUEST', 'No valid RSVPs for this event', 400)

    // Capacity enforcement for "approved"
    let toUpdate = validIds
    let skipped: string[] = []
    if (statusRaw === 'approved' && typeof event.capacity === 'number' && event.capacity > 0) {
      const [{ c: approvedCount }] = await db
        .select({ c: sql<number>`count(*)` })
        .from(schema.rsvps)
        .where(and(eq(schema.rsvps.eventId, event.id), eq(schema.rsvps.status, 'approved')))
      const remaining = Math.max(0, Number(event.capacity) - Number(approvedCount || 0))
      if (remaining <= 0) {
        return apiSuccess({ ok: true, updated: 0, skipped: validIds, reason: 'capacity_full' })
      }
    ...
    if (toUpdate.length) {
      await db.update(schema.rsvps).set({ status: statusRaw as any }).where(inArray(schema.rsvps.id, toUpdate))
    }
