...
export async function GET(req: NextRequest) {
  try {
    ...
    const sortBy = (searchParams.get("sortBy") || "date").trim() as
      | "date"
      | "title";
    const sortDir = (searchParams.get("sortDir") || "desc").trim() as
      | "asc"
      | "desc";
    ...
    const orderExpr =
      sortBy === "title"
        ? sql`${schema.posts.title} ${sql.raw(sortDir)}`
        : sql`${schema.posts.date} ${sql.raw(sortDir)}`;
