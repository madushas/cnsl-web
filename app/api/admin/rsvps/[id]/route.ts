...
export async function PATCH(
  ...
  try {
    ...
    const ticketNumber =
      typeof ticketNumberRaw === "string" && ticketNumberRaw.trim()
        ? String(ticketNumberRaw)
            .trim()
            .toUpperCase()
            .replace(/[^A-Z0-9\-]/g, "")
            .slice(0, 64)
    ...
    if (status === undefined && ticketNumber === undefined)
    ...
    await db.update(schema.rsvps).set(setUpdate).where(eq(schema.rsvps.id, id));
    ...
    await logAudit({
      ...
      oldValues: {
        ...
        previousTicketNumber: r.ticketNumberPrev,
      },
    ...
    if (notify && r.email && status) {
      ...
      await sendTelegram(
        `RSVP ${status}: ${r.name || r.email} -> ${r.eventTitle}`,
      );
    }
    ...
    return NextResponse.json(
      { error: "Unable to update RSVP. Please try again." },
      { status: 500 },
    );
  }
}
