...
// Generate QR code with CDN upload
async function generateQRCodeWithCDN(data: string, ticketNumber: string): Promise<string> {
  try {
    // Use the updated generateQRCode function from ticket-generator
    const { generateQRCode } = await import('@/lib/ticket-generator');
    return await generateQRCode(data, ticketNumber, 400, "H");
  } catch (error) {
    ...
    if (action === "send_invites") {
      ...
      if (!rateCheck.allowed) {
        ...
        return NextResponse.json(
          {
            error:
              "Rate limit exceeded. QR generation is resource-intensive. Please wait 30 seconds.",
            resetAt: rateCheck.resetAt.toISOString(),
          },
          { status: 429 },
        );
      ...
      for (const rsvp of rsvpList) {
        ...
        const qrCodeUrl = await generateQRCodeWithCDN(qrPayload, ticketNumber);
        updates.push({ id: rsvp.id, qrCode: qrCodeUrl, ticketNumber });
      }

      // Apply DB updates (removed transaction as neon-http doesn't support it)
      const now = new Date();
