...
export async function POST(req: NextRequest) {
  try {
    ...
    if (id) {
      const row = await db.select({ id: schema.rsvps.id, checkedInAt: schema.rsvps.checkedInAt }).from(schema.rsvps).where(and(eq(schema.rsvps.id, id), eq(schema.rsvps.eventId, event.id))).limit(1)
      rsvp = row[0] || null
    } else if (ticketNumber) {
      const row = await db.select({ id: schema.rsvps.id, checkedInAt: schema.rsvps.checkedInAt }).from(schema.rsvps).where(and(eq(schema.rsvps.eventId, event.id), eq(schema.rsvps.ticketNumber, ticketNumber))).limit(1)
      rsvp = row[0] || null
    } else if (qr) {
      const row = await db.select({ id: schema.rsvps.id, checkedInAt: schema.rsvps.checkedInAt }).from(schema.rsvps).where(and(eq(schema.rsvps.eventId, event.id), eq(schema.rsvps.qrCode, qr))).limit(1)
      rsvp = row[0] || null
    } else if (email) {
      const row = await db.select({ id: schema.rsvps.id, checkedInAt: schema.rsvps.checkedInAt }).from(schema.rsvps).where(and(eq(schema.rsvps.eventId, event.id), eq(schema.rsvps.email, email))).limit(1)
      rsvp = row[0] || null
    }
    ...
    await db.update(schema.rsvps).set({ checkedInAt: new Date() }).where(eq(schema.rsvps.id, rsvp.id))
