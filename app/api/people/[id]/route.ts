...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [oldPerson] = await db.select().from(schema.people).where(eq(schema.people.id, id)).limit(1)
    await db.update(schema.people)
      .set({
        ...
        company: p.company ? (DOMPurify.sanitize(String(p.company)) as any) : undefined,
        linkedin: p.linkedin ? (DOMPurify.sanitize(String(p.linkedin)) as any) : undefined,
        twitter: p.twitter ? (DOMPurify.sanitize(String(p.twitter)) as any) : undefined,
        github: p.github ? (DOMPurify.sanitize(String(p.github)) as any) : undefined,
        website: p.website ? (DOMPurify.sanitize(String(p.website)) as any) : undefined,
        photo: p.photo ? (DOMPurify.sanitize(String(p.photo)) as any) : undefined,
        category: p.category ? (DOMPurify.sanitize(String(p.category)) as any) : undefined,
      })
      .where(eq(schema.people.id, id))
    const [newPerson] = await db.select().from(schema.people).where(eq(schema.people.id, id)).limit(1)
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await db
      ...
      .set({ deletedAt: new Date(), deletedBy: admin?.id ? String(admin.id) : null })
      .where(eq(schema.people.id, id))
    await logAudit({
