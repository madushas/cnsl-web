...
export async function GET(
  ...
  const [row] = await db
    ...
    .from(schema.posts)
    .where(
      sql`${schema.posts.slug} = ${slug} AND ${schema.posts.deletedAt} IS NULL`,
    )
    .limit(1);
    ...
    await db
      ...
      .set({
        title: p.title
          ...
          : undefined,
        excerpt: p.excerpt
          ? (DOMPurify.sanitize(String(p.excerpt)) as any)
          : undefined,
        category: p.category
          ? (DOMPurify.sanitize(String(p.category)) as any)
          : undefined,
        image: p.image
          ? (DOMPurify.sanitize(String(p.image)) as any)
          : undefined,
        date: p.date ? new Date(String(p.date)) : undefined,
        author: p.author
          ? (DOMPurify.sanitize(String(p.author)) as any)
          : undefined,
        tags: tags ? (DOMPurify.sanitize(String(tags)) as any) : undefined,
        content: content
          ? (DOMPurify.sanitize(String(content)) as any)
          : undefined,
