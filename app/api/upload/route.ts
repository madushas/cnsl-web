...
export async function POST(req: NextRequest) {
  try {
    ...
    if (file.size > MAX_FILE_SIZE) {
      return apiError('VALIDATION_ERROR', `File size exceeds ${MAX_FILE_SIZE / 1024 / 1024}MB limit`, 400)
    }
    ...
    if (!ALLOWED_MIME_TYPES.includes(file.type)) {
      return apiError('VALIDATION_ERROR', `Invalid file type. Allowed types: ${ALLOWED_EXTENSIONS.join(', ')}`, 400)
    }
    ...
    if (!hasValidExtension) {
      return apiError('VALIDATION_ERROR', `Invalid file extension. Allowed: ${ALLOWED_EXTENSIONS.join(', ')}`, 400)
    }
