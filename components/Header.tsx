...
export function Header() {
  ...
  useEffect(() => {
    ...
    try {
      if (typeof window !== 'undefined' && sessionStorage.getItem('meSynced') === '1') return
      fetch('/api/me', { cache: 'no-store' }).catch(() => {})
      if (typeof window !== 'undefined') sessionStorage.setItem('meSynced', '1')
    } catch {}
  }, [user])
    ...
    <header className="sticky top-0 z-50 border-b border-border bg-background/95 backdrop-blur-sm supports-[backdrop-filter]:bg-background/90">
      <div className="container mx-auto flex h-16 md:h-20 items-center justify-between px-4">
        ...
        <div className="flex items-center gap-2 md:gap-4">
          {/* Desktop Search */}
          <div role="search" aria-label="Site search" className="relative hidden xl:block">
          ...
          {user ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="hidden sm:flex items-center gap-2 h-10 hover:bg-surface-subtle">
                  ...
                  <span className="text-sm text-white/90">{user.displayName || "Account"}</span>
                </Button>
              </DropdownMenuTrigger>
                ...
                {navigationLinks.map((link) => (
                  <Link
                    ...
                    className={`rounded-lg px-4 py-3 text-base font-medium transition-all hover:bg-surface-subtle ${
                      pathname === link.href ? 'text-blue-400' : 'text-white/90 hover:text-blue-400'
                    }`}
                  >
                    {link.label}
                  </Link>
              ...
              <div className="border-t border-border pt-6 space-y-3">
                {user ? (
                  <>
                    ...
                    <Button asChild variant="outline" className="w-full border-white/20 text-white hover:bg-surface-subtle">
