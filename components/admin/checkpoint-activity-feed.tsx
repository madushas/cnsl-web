  ...
  const fetchHistory = async (signal?: AbortSignal) => {
    try {
      ...
      const res = await fetch(
        `/api/admin/events/${encodeURIComponent(slug)}/checkpoints/history?${params.toString()}`,
              ...
              {items.map((it) => {
                ...
                return (
                  <li key={it.id} className="py-3 flex items-start gap-3">
                    <div
                      className={`shrink-0 h-8 w-8 flex items-center justify-center rounded-md ${colors.bg}`}
                    >
                      <span
                        className="text-base"
                        role="img"
                        aria-label={it.checkpointType}
                      >
                        {CheckpointIcons[it.checkpointType]}
                      </span>
                    </div>
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 text-xs text-muted-foreground flex items-center gap-2 flex-wrap">
                        ...
                        {it.scannedBy ? (
                          <span>
                            by <span className="font-mono">{it.scannedBy}</span>
                          </span>
                        ) : null}
                        <span>
                          at {new Date(it.scannedAt).toLocaleString()}
                        </span>
                        {it.notes ? (
