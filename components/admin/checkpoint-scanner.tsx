...
import { CheckpointType, ScanMethod, CheckpointLabels, CheckpointIcons } from '@/lib/types/checkpoint'
...
export default function CheckpointScanner({ slug }: { slug: string }) {
  ...
  const [activeCheckpoint, setActiveCheckpoint] = useState<CheckpointType>('entry')
  const [stats, setStats] = useState<ScanStats>({ entry: 0, refreshment: 0, swag: 0 })
        ...
        const scanner = new QrScanner(
          ...
          (result) => {
            ...
            if (!prev || prev.value !== raw || now - prev.ts > 2000) {
              console.log('[Scanner] New/unique scan, calling scanCheckpoint')
  ...
  const scanCheckpoint = useCallback(async (
    identifier: { id?: string; ticketNumber?: string; email?: string; qr?: string },
    scanMethod: ScanMethod
  ) => {
    const key = identifier.id || identifier.ticketNumber || identifier.email || identifier.qr
    console.log('[Scanner] scanCheckpoint called:', { key, checkpoint: activeCheckpoint })
    
    if (busy) {
      console.log('[Scanner] Scan skipped: already busy')
      return
    }
    
    setBusy(true)
    try {
      const res = await fetch(`/api/admin/events/${encodeURIComponent(slug)}/checkpoints`, {
        method: 'POST',
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>
              Scan QR codes for {CheckpointLabels[activeCheckpoint].toLowerCase()}
            </CardDescription>
          </CardHeader>
              ...
              {streaming && (
                <div className="text-xs flex items-center gap-2">
                  <span className={scannerActive ? 'text-green-500' : 'text-yellow-500'}>
                    {scannerActive ? '● Scanner active' : '○ Scanner unavailable'}
                  </span>
                  {lastDetection && (
                    <span className="text-muted-foreground">Last: {lastDetection}...</span>
                  )}
                </div>
              )}
            </div>
          ...
          <CardContent className="space-y-3">
            <div>
              ...
              <Input 
                ...
                onChange={e => setManual(p => ({ ...p, ticketNumber: e.target.value }))} 
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && manual.ticketNumber) {
                    scanCheckpoint({ ticketNumber: manual.ticketNumber }, 'ticket')
                  }
                }}
            ...
            <div>
              ...
              <Input 
                value={manual.qr} 
                onChange={e => setManual(p => ({ ...p, qr: e.target.value }))} 
                onKeyDown={(e) => {
            ...
            <Button 
              disabled={busy || (!manual.ticketNumber && !manual.email && !manual.qr)} 
              onClick={() => {
                if (manual.ticketNumber) scanCheckpoint({ ticketNumber: manual.ticketNumber }, 'ticket')
                else if (manual.email) scanCheckpoint({ email: manual.email }, 'email')
              ...
              <div className="divide-y divide-border text-xs">
                ...
                {logs.map((e, idx) => (
                  <div key={idx} className="p-2 flex items-start gap-2">
                    ...
                    {e.result === 'success' && <Check className="h-4 w-4 text-green-500 shrink-0" />}
                    {e.result === 'already_scanned' && <AlertCircle className="h-4 w-4 text-yellow-500 shrink-0" />}
                    {e.result === 'not_found' && <X className="h-4 w-4 text-orange-500 shrink-0" />}
                    {e.result === 'error' && <X className="h-4 w-4 text-red-500 shrink-0" />}
                    <div className="flex-1 min-w-0">
                      {e.checkpointType && (
                        <span className="mr-1">{CheckpointIcons[e.checkpointType]}</span>
                      )}
                      {e.attendeeName && (
