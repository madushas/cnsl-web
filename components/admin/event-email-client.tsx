...
export default function EventEmailClient({ slug, eventTitle }: Props) {
  ...
  const [subject, setSubject] = useState<string>(`You're invited: {{eventTitle}}`)
  const [preheader, setPreheader] = useState<string>('We are excited to see you at {{eventTitle}}')
...
<html>
  ...
  const [previewTarget, setPreviewTarget] = useState<BulkEmailTarget | null>(null)
  ...
  async function doPreview() {
    try {
      ...
      const res = await fetch(`/api/admin/events/${slug}/preview-email`, {
        ...
        body: JSON.stringify({ subject, preheader, html, q, status: statusParam }),
      })
      ...
      toast({ title: 'Preview ready', description: `Sample of ${data.total} potential recipients` })
    } catch (e) {
  ...
  async function startNotify() {
    try {
      ...
      const res = await fetch(`/api/admin/events/${slug}/notify`, {
        method: 'POST',
        headers: withCSRF({ 'Content-Type': 'application/json' }),
        body: JSON.stringify({ subject, preheader, html, q, status: statusParam, ratePerMinute: rate }),
      })
      const data = await res.json()
      ...
      toast({ title: 'Job started', description: `Sending to ${data.total} recipients at ${rate}/min` })
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Compose, preview, and send bulk emails for {eventTitle}</p>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div>
              ...
              <Textarea className="min-h-[280px] font-mono text-sm" value={html} onChange={e=>setHtml(e.target.value)} />
            </div>
            <div className="grid grid-cols-3 gap-3">
              <div className="col-span-2">
                <label className="text-xs block mb-1">Filter: Search</label>
                <Input placeholder="Search (name/email/affiliation)" value={q} onChange={e=>setQ(e.target.value)} />
              </div>
              <div>
            ...
            <div className="flex items-center gap-3">
              <Button onClick={doPreview} variant="outline">Preview</Button>
              <div className="text-sm text-muted-foreground">{previewCount ? `${previewCount} potential recipients` : ''}</div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div>
              ...
              <div className="text-sm font-medium">{previewSubject || subject}</div>
            </div>
            <div className="border rounded p-3 bg-background" dangerouslySetInnerHTML={{ __html: previewHtml || html }} />
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Start a background job and monitor live progress</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="grid sm:grid-cols-3 gap-3 items-end">
            <div>
              <label className="text-xs block mb-1">Rate (emails/min)</label>
              <Input type="number" min={1} max={60} value={rate} onChange={e=>setRate(Math.max(1, Math.min(60, Number(e.target.value||'2'))))} />
            </div>
            <div className="sm:col-span-2 flex items-center gap-2">
              <Button onClick={startNotify}>Start Send</Button>
              <Button variant="outline" onClick={cancelJob} disabled={!jobId || jobStatus !== 'running'}>Cancel Job</Button>
            </div>
          </div>
          {jobId && (
            <div className="text-sm">
              <div>Job: <span className="font-mono">{jobId}</span> · Status: <span className="font-medium">{jobStatus}</span></div>
              <div className="h-2 bg-muted rounded overflow-hidden mt-2">
                <div className="bg-blue-600 h-full" style={{ width: `${jobTotal ? Math.round(100*jobProgress/jobTotal) : 0}%` }} />
              </div>
              <div className="text-xs text-muted-foreground mt-1">{jobProgress} / {jobTotal}</div>
            </div>
          )}
        </CardContent>
      </Card>
