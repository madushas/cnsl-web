...
export function EventFormAdvanced({
  ...
  mode: "create" | "edit";
  initial?: Partial<EventValue> | null;
  onSaved?: () => void;
}) {
  ...
  const [form, setForm] = useState<EventValue>(() => ({
    ...
    date: initial?.date
      ? new Date(initial.date).toISOString()
  ...
  useEffect(() => {
    ...
    setForm((prev) => ({
      ...
      capacity:
        typeof initial.capacity === "number" ? initial.capacity : prev.capacity,
      published:
        typeof initial.published === "boolean"
          ? initial.published
          : prev.published,
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (mode === "create") {
        ...
        const res = await fetch(`/api/events/${form.slug}`, {
          method: "PATCH",
          headers: withCSRF({ "Content-Type": "application/json" }),
          body: JSON.stringify(payload),
        });
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <div className="grid gap-2">
              ...
              <Input
                ...
                disabled={mode === "edit"}
                placeholder="cnsl-oct-2025-colombo"
              />
              {formErrors.slug && (
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                placeholder="CNSL October Meetup"
              />
              {formErrors.title && (
            ...
            <div className="grid gap-2">
              ...
              />
              {formErrors.city && (
                <div className="text-xs text-red-400">{formErrors.city}</div>
              )}
            </div>
            <div className="grid gap-2">
              ...
              <Input
                ...
                placeholder="WSO2 Auditorium"
              />
              {formErrors.venue && (
                <div className="text-xs text-red-400">{formErrors.venue}</div>
              )}
            </div>
            <div className="grid gap-2">
            ...
            <div className="grid gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">
                  Visible on site when published
                </span>
              </div>
            </div>
          </div>
