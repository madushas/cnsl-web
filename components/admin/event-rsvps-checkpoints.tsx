...
export default function EventRSVPsCheckpoints({ slug }: Props) {
  ...
  async function fetchData(signal?: AbortSignal) {
    try {
      ...
      if (checkpointFilter !== "all")
        params.set("checkpoint", checkpointFilter);
      ...
      if (!res.ok)
        throw new Error(
          data?.error?.message || data?.error || "Failed to load",
        );
  ...
  async function bulkCheckpoint(
    action: "mark" | "undo",
    checkpointType: "entry" | "refreshment" | "swag",
  ) {
    if (!selectedRsvps.size) {
      toast({
        title: "No selection",
        description: "Select RSVPs first.",
        variant: "destructive",
      });
      return;
    }
    try {
      const res = await fetch(`/api/admin/events/${slug}/checkpoints/bulk`, {
        ...
        body: JSON.stringify({
          rsvpIds: Array.from(selectedRsvps),
          action,
          checkpointType,
          scanMethod: "manual",
        }),
      ...
      if (!res.ok)
        throw new Error(
          data?.error?.message || data?.error || "Failed to start bulk job",
        );
      ...
      toast({
        title: "Error",
        description: e.message || "Failed to start bulk job",
        variant: "destructive",
      });
    }
  }
  ...
  function startBulkStream(jobId: string) {
    ...
    const onStatus = (evt: MessageEvent) => {
      try {
        ...
        setBulkJob((j) => ({
          id: jobId,
          status: String(job.status || j?.status || "running"),
          progress: Number(job.progress || 0),
          total: Number(job.total || j?.total || 0),
          successCount: job.meta?.successCount,
          errorCount: job.meta?.errorCount,
          skippedCount: job.meta?.skippedCount,
        }));
        if (
  ...
  async function markCheckpoint(rsvpId: string, checkpointType: string) {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(
          data?.error?.message || data?.error || "Failed to mark checkpoint",
        );
      toast({
        title: "Checkpoint Marked",
        description: `Successfully marked ${checkpointType}`,
      });
      await fetchData();
    } catch (error: any) {
      toast({
  ...
  async function undoCheckpoint(rsvpId: string, checkpointType: string) {
    ...
    try {
      const res = await fetch(
        `/api/admin/events/${slug}/rsvps/${rsvpId}/checkpoints/${checkpointType}`,
        {
          method: "DELETE",
          headers: withCSRF(),
        },
      ...
      if (!res.ok)
        throw new Error(
          data?.error?.message || data?.error || "Failed to undo checkpoint",
        );
      toast({
        title: "Checkpoint Undone",
        description: `Successfully undone ${checkpointType}`,
      });
      await fetchData();
    } catch (error: any) {
      toast({
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="missing-refreshment">
                  Missing Refreshment
                </SelectItem>
            ...
            <div className="flex items-center gap-2 p-3 bg-blue-50 dark:bg-blue-950 rounded-lg">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem
                    onClick={() => bulkCheckpoint("mark", "entry")}
                  >
                    Mark Entry
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onClick={() => bulkCheckpoint("mark", "refreshment")}
                  >
                    Mark Refreshment
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onClick={() => bulkCheckpoint("mark", "swag")}
                  >
                    Mark Swag
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
                  <DropdownMenuItem
                    onClick={() => bulkCheckpoint("undo", "entry")}
                  >
                    Undo Entry
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onClick={() => bulkCheckpoint("undo", "refreshment")}
                  >
                    Undo Refreshment
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onClick={() => bulkCheckpoint("undo", "swag")}
                  >
                    Undo Swag
                  </DropdownMenuItem>
                </DropdownMenuContent>
                  ...
                  rsvps.map((rsvp) => (
                    <TableRow key={rsvp.id}>
                      ...
                      <TableCell>
                        <Badge
                          variant={
                            rsvp.status === "approved" ? "default" : "outline"
                          }
                        >
                          {rsvp.status}
                        </Badge>
                      ...
                      <TableCell className="text-center">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem
                              onClick={() => markCheckpoint(rsvp.id, "entry")}
                            ...
                            <DropdownMenuItem
                              onClick={() =>
                                markCheckpoint(rsvp.id, "refreshment")
                              }
                            ...
                            <DropdownMenuItem
                              onClick={() => markCheckpoint(rsvp.id, "swag")}
                            ...
                            <DropdownMenuSeparator />
                            <DropdownMenuItem
                              onClick={() => undoCheckpoint(rsvp.id, "entry")}
                              disabled={!rsvp.checkpoints?.hasEntry}
                            >
                              Undo Entry
                            </DropdownMenuItem>
                            <DropdownMenuItem
                              onClick={() =>
                                undoCheckpoint(rsvp.id, "refreshment")
                              }
                              disabled={!rsvp.checkpoints?.hasRefreshment}
                            >
                              Undo Refreshment
                            ...
                            <DropdownMenuItem
                              onClick={() => undoCheckpoint(rsvp.id, "swag")}
                              disabled={!rsvp.checkpoints?.hasSwag}
                            >
                              Undo Swag
                            </DropdownMenuItem>
      ...
      >
        <DialogContent>
          ...
          <div className="space-y-2 text-sm">
            <div>
              Status:{" "}
              <span className="font-mono">{bulkJob?.status || "..."}</span>
            </div>
            <div>
              Progress: {bulkJob?.progress || 0} / {bulkJob?.total || 0}
            </div>
            <Progress
              value={Math.round(
                ((bulkJob?.progress || 0) * 100) /
                  Math.max(1, bulkJob?.total || 1),
              )}
            />
            {(bulkJob?.successCount != null || bulkJob?.errorCount != null) && (
              <div>
                Success: {bulkJob?.successCount ?? 0} · Errors:{" "}
                {bulkJob?.errorCount ?? 0}
                {bulkJob?.skippedCount != null
                  ? ` · Skipped: ${bulkJob?.skippedCount}`
                  : ""}
              </div>
            )}
          </div>
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  if (!bulkJob) return;
                  const res = await fetch(
                    `/api/admin/jobs/${bulkJob.id}/cancel`,
                  ...
                  toast({
                    title: "Cancel failed",
                    description: e.message || "Unable to cancel job",
                    variant: "destructive",
                  });
                }
              }}
              disabled={
                !bulkJob ||
                ["completed", "failed", "cancelled"].includes(bulkJob.status)
              }
            >
              Cancel Job
            </Button>
