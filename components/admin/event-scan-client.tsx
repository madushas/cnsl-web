...
export default function EventScanClient({ slug }: { slug: string }) {
        ...
        const scanner = new QrScanner(
          ...
          (result) => {
            ...
            if (!prev || prev.value !== raw || now - prev.ts > 2000) {
              ...
              console.log(
                "[Scanner] Duplicate scan ignored (within 2s window)",
              );
            }
          },
          {
  ...
  async function checkIn(partial: {
    id?: string;
    ticketNumber?: string;
    email?: string;
    qr?: string;
    ...
    const key =
      partial.id || partial.ticketNumber || partial.email || partial.qr;
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/events/${encodeURIComponent(slug)}/checkins/bulk`,
        {
          method: "POST",
      ...
      if (item?.updated) {
        ...
        toast({
          title: "Checked in",
          description: String(key || "").slice(0, 64),
        });
        try {
  ...
  async function uncheck() {
    ...
    try {
      ...
      if (r?.updated) {
        ...
        toast({
          title: "Not found",
          description: "Could not match previous check-in",
          variant: "destructive",
        });
      }
    } catch (e: any) {
      toast({
        title: "Error",
        description: e.message || "Failed to uncheck",
        variant: "destructive",
      });
    }
  }
    ...
    <div className="grid lg:grid-cols-2 gap-6">
        ...
        <CardContent className="space-y-3">
            ...
            {streaming && (
              <div className="text-xs">
                ...
                >
                  {scannerActive
                    ? "● Scanner active"
                    : "○ Scanner unavailable (use manual entry)"}
                </span>
                {lastDetection && (
          ...
          {!streaming && (
            <div className="text-xs text-muted-foreground">
              Tip: open the site on http://localhost:3000 (or HTTPS). Browsers
              block camera access on plain HTTP over LAN IPs.
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Check-in by ticket number, email, or QR URL
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div>
            ...
            <Input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter")
                  checkIn({ ticketNumber: manual.ticketNumber });
              }}
            />
          </div>
          <div>
            ...
            <Input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") checkIn({ email: manual.email });
              }}
            />
          </div>
          <div>
            ...
            <Input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") checkIn({ qr: manual.qr });
              }}
            />
          </div>
          <div className="flex items-center gap-2">
            <Button
              disabled={busy}
              onClick={() => checkIn({ ticketNumber: manual.ticketNumber })}
            >
              Check-in
            </Button>
            <Button variant="outline" disabled={!last?.key} onClick={uncheck}>
              Undo last
            </Button>
          </div>
        </CardContent>
