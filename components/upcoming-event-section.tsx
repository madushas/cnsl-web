  ...
  return (
    <section className="container mx-auto px-4 section-spacing">
      <Reveal className="mb-12 md:mb-16 text-center">
        <h2 className="mb-4 text-h2 text-foreground">Upcoming Event Spotlight</h2>
        <p className="text-lg md:text-xl text-muted-foreground">Don’t miss our next community gathering</p>
      </Reveal>
          ...
          <CardContent className="relative p-0">
            <div className="grid items-stretch gap-0 lg:grid-cols-12">
              ...
              <div className="col-span-12 lg:col-span-6 card-padding-lg lg:p-12 space-y-6">
                <div>
                  <Badge variant="secondary" className="mb-2 gap-2 px-4 py-1.5 text-sm">
                    <Calendar className="h-4 w-4" />
                    Next Up
                  </Badge>
                  {nextUp && (
                    <>
                      <h3 className="mt-4 text-h2 text-foreground">{nextUp.title}</h3>
                      <div className="flex items-center gap-2 text-muted-foreground">
                        <MapPin className="h-4 w-4" />
                        <span>{nextUp.venue} — {nextUp.city}</span>
                      </div>
                      <div className="text-xs text-muted-foreground">Sessions: {nextUp.topics.length} • Speakers: {nextUp.speakers.length}</div>
                      <div className="mt-1 text-xs text-muted-foreground">Registration is open. Due to space limitations, RSVP emails may be sent to selected participants.</div>
                    </>
                  )}
                </div>
                ...
                <div className="flex flex-wrap gap-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="font-semibold text-foreground">{dt ? dt.toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' }) : '-'}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="rounded-lg border border-border bg-surface-subtle p-2">
                    ...
                    <div>
                      ...
                      <div className="font-semibold text-foreground">{nextUp?.venue}</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between gap-6 pt-4">
                  <div className="text-left">
                    <div className="text-2xl font-bold text-foreground">{nextUp?.registered ?? 0}+</div>
                  ...
                  >
                    <Link href={nextUp ? `/events/${nextUp.slug}` : "/events"} aria-label="Register for the next CNSL event">
                      Register
                    </Link>
                  </Button>
                </div>
