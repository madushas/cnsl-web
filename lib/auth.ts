...
export async function getSessionUser(): Promise<SessionUser> {
  try {
    // Diagnostic logging: check stackServerApp presence
    try {
      console.debug('[lib/auth] stackServerApp present:', !!stackServerApp)
    } catch {}
    const u = await stackServerApp?.getUser()
    if (!u) return null
    // Normalize common fields defensively (only serializable props)
    ...
    // Note: Do NOT include the original user object (contains functions like signOut)
    // Passing functions to Client Components causes serialization errors
    return { id, email, name, imageUrl }
  } catch (error) {
    logger.error('Session validation failed', { error: error instanceof Error ? error : String(error) })
    try { console.error('[lib/auth] getSessionUser error:', error) } catch {}
    return null
  }
}
...
export async function getUserRoles(user: SessionUser): Promise<string[]> {
  ...
  if (hasId || hasEmail) {
    const cond = hasId && hasEmail
      ...
      ? eq(schema.userRoles.userId, String(user!.id))
      : ilike(schema.userRoles.email, String(user!.email!))
    rows = await db.select({ role: schema.userRoles.role })
      .from(schema.userRoles)
      .where(cond)
  }
  // Normalize DB roles: trim + lowercase (e.g., ' ADMIN ' => 'admin')
  const roles = rows.map(r => String(r.role || '').trim().toLowerCase())
  const envAdmins = (process.env.ADMIN_EMAILS || '').split(',').map(s=>s.trim().toLowerCase()).filter(Boolean)
  if (user?.email && envAdmins.includes(user.email.toLowerCase()) && !roles.includes('admin')) {
    roles.push('admin')
  }
  return roles
}
...
export async function requireAdminOrCheckin() {
  ...
  if (!user) throw new Error('Unauthorized')
  const roles = await getUserRoles(user)
  if (!roles.includes('admin') && !roles.includes('checkin')) throw new Error('Forbidden')
  return user
}
