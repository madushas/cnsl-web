...
function genId() {
  try { return crypto.randomUUID() } catch { return 'job_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2,8) }
}
...
async function redisSetJSON(key: string, value: unknown) {
  ...
  try {
    ...
    await fetch(`${REDIS_URL}/set/${encodeURIComponent(key)}/${encodeURIComponent(body)}`, {
      method: 'POST',
...
async function redisGetJSON<T = unknown>(key: string): Promise<T | undefined> {
  ...
  try {
    ...
    try { return JSON.parse(data.result) as T } catch { return undefined }
...
export function createBulkEmailJob(payload: BulkEmailPayload, meta?: Record<string, unknown>) {
  const id = genId()
  const job: Job = {
    id,
    type: 'bulkEmail',
...
export function createBulkCheckpointJob(payload: BulkCheckpointPayload, meta?: Record<string, unknown>) {
  const id = genId()
  const job: Job = {
    id,
    type: 'bulkCheckpoint',
...
function renderTemplate(html: string, preheader: string | undefined, t: BulkEmailTarget, eventTitle?: string | null) {
  let out = html
  const map: Record<string, string> = {
...
export function renderBulkEmailTemplate(subject: string, preheader: string | undefined, html: string, t: BulkEmailTarget, eventTitle?: string | null) {
...
export async function runBulkEmailJob(jobId: string, payload: BulkEmailPayload, eventTitle?: string | null) {
  ...
  const intervalMs = Math.max(1000, Math.floor(60000 / Math.max(1, payload.ratePerMinute || 2)))

  try {
    const failures: Array<{ email: string; reason: string }> = []
    for (let i = 0; i < payload.targets.length; i++) {
      if (job.cancelled) { job.status = 'cancelled'; job.finishedAt = Date.now(); job.updatedAt = Date.now(); putJob(job); return }

      const target = payload.targets[i]
      const html = renderTemplate(payload.html, payload.preheader, target, eventTitle)
      const subject = renderTemplate(payload.subject, payload.preheader, target, eventTitle)
      ...
      if (failures.length) {
        ...
        job.failureReasons = Object.fromEntries(failures.map(f => [f.email, f.reason]))
      }
      await putJob(job)
      broadcast(job.id, { type: 'progress', progress: job.progress, total: job.total })
...
export async function retryFailedEmails(jobId: string): Promise<string | null> {
  ...
  const meta = job.meta || {}
  const allTargets: BulkEmailTarget[] = Array.isArray(meta.targets) ? meta.targets : []
