...
export async function sendEmail(opts: { to?: string, subject: string, html: string }) {
  ...
  await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({ from: FROM_EMAIL, to, subject: opts.subject, html: opts.html }),
  }).catch(() => {})
}
...
export async function sendTelegram(text: string) {
  ...
  await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body }).catch(() => {})
}
