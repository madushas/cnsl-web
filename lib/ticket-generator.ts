...
export type TextOverlay = {
  field: 'name' | 'ticketNumber' | 'email' | 'eventTitle' | 'eventDate' | 'venue';
  x: number;
  y: number;
  fontSize: number;
  fontFamily: string;
...
function drawText(
  ...
  const { x, y, fontSize, fontFamily, color, align = 'left', maxWidth } = overlay;
...
export async function generateTicketsBatch(
  ...
  for (let i = 0; i < dataList.length; i++) {
    ...
    try {
      ...
      console.error(`Failed to generate ticket for ${data.ticketNumber}:`, error);
  ...
  console.log(`Batch complete: ${results.size}/${dataList.length} tickets in ${((endTime - startTime) / 1000).toFixed(2)}s (avg ${avgTime.toFixed(2)}ms/ticket)`);
 ...
 */
export function getDefaultTemplate(): Omit<TicketTemplate, 'id' | 'name' | 'backgroundImage'> {
  return {
    qrConfig: {
      x: 50,
      y: 50,
