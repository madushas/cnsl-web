...
const checkinItemSchema = z
  ...
  .refine((d) => d.id || d.email || d.ticketNumber || d.qr, {
    message:
      "At least one identifier (id, email, ticketNumber, or qr) is required",
  });

export const bulkCheckinsSchema = z.object({
  action: z.enum(["checkin", "uncheck"]),
  items: z
    .array(checkinItemSchema)
    .min(1, "items array is required")
    .max(500, "Maximum 500 items allowed per bulk operation"),
});
...
export const EventInput = z.object({
  ...
  slug: z
    ...
    .regex(
      /^[a-z0-9]+(?:-[a-z0-9]+)*$/,
      "Invalid slug format (use lowercase and hyphens)",
    ),
...
export const adminSelectSchema = z.object({
  ...
  rsvpIds: z
    .array(z.uuid())
    .min(1, "No RSVP IDs provided")
    .max(1000, "Maximum 1000 RSVPs per bulk operation"),
});
...
export const previewEmailSchema = z.object({
  ...
  ids: z
    .array(z.uuid())
    .max(100, "Maximum 100 preview recipients")
    .optional()
    .default([]),
});
...
export const notifySchema = z.object({
  ...
  ids: z
    .array(z.uuid())
    .max(5000, "Maximum 5000 recipients per bulk email")
    .optional()
    .default([]),
...
export const checkpointScanSchema = z.object({
  ...
  identifier: z
    ...
    .refine((data) => data.id || data.email || data.ticketNumber || data.qr, {
      message:
        "At least one identifier (id, email, ticketNumber, or qr) is required",
    })
    .optional(),
  ...
  items: z
    .array(
      z
        ...
        .refine((data) => data.rsvpId || data.email || data.ticketNumber, {
          message: "At least one identifier required per item",
        }),
    )
    .min(1, "At least one item required")
    .max(100, "Maximum 100 items per bulk scan"),
});
