...
const RATE_LIMIT_ENABLED =
  isProduction && process.env.DISABLE_RATE_LIMIT !== "true";
...
if (RATE_LIMIT_ENABLED) {
  ...
  if (
    ...
    try {
      ...
      console.warn(
        "⚠️ Upstash Redis initialization failed, falling back to in-memory rate limiting",
        error,
      );
    }
  } else {
    console.log(
      "ℹ️  Using in-memory rate limiting (set UPSTASH_REDIS_REST_URL for production)",
    );
  }
} else {
...
export async function middleware(request: NextRequest) {
  ...
  // Centralized sanitized request logging for API endpoints
  try {
    if (isApi) {
      // Delegate to centralized request logger which sanitizes and emits structured logs.
      // The logger implementation handles redaction of sensitive headers/fields.
      try {
        logRequest(request);
      } catch (err) {
        // Keep this very small to avoid throwing from logging code
        console.warn(
          "[middleware] request-logger failed",
    ...
    try {
      console.warn(
        "[middleware] Failed to log request",
    ...
    if (
      ...
      try {
        ...
        if (!originHost || originHost !== host) {
          console.warn(
            `[CSRF_FAIL] origin=${originHost}, host=${host}, path=${pathname}`,
          );
    ...
    if (RATE_LIMIT_ENABLED && !isAdminPath && !pathname.startsWith("/api/me")) {
      try {
        ...
        if (meRes.ok) {
          ...
          const userId: string | undefined = mePayload?.user?.id
            ? String(mePayload.user.id)
            : undefined;
          if (userId) {
  ...
  if (isAdminPath) {
    try {
      ...
      const mePayload = meJson?.data ?? meJson;
      const roles: string[] = Array.isArray(mePayload?.roles)
        ? mePayload.roles
        : [];
      ...
      const allowCheckin =
        (isScannerUI || isCheckinAPI) && roles.includes("checkin");
